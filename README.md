...
- **智能缓存**：结果缓存、评论人数缓存与插件本地缓存，避免重复请求
- **长任务提示**：按时间窗口设置合理等待时间，冷启动抓取期间不会被浏览器提前中断
...
运行期间如果登录令牌过期，后端会自动刷新登录态并重试一次。若北大登录系统再次要求手机令牌，API 会明确提示重新执行上述交互式登录命令。
...
5. 插件安装完成：
   ...
   - **侧边栏**：访问 `treehole.pku.edu.cn` 时，页面右侧出现 🔥 按钮；首次展开自动加载热帖，窗口选择和上次结果会在插件内保存
